...
MANIFEST="/home/pham156/MoE/FuseMoE_poly/out/Week_28/router_init_diagnostics/week28_router_init_manifest.tsv"
...
for task_idx in "${!TASKS[@]}"; do
  ...
  for seed in "${SEEDS[@]}"; do
    for variant in "${VARIANTS[@]}"; do
      ...
      config_id="w28_${short_task}_s${seed}_${short_variant}"
      submit_output=$(
        CONFIG_ID="$config_id" \
        TASK="$task" \
        NUM_LABELS="$labels" \
        ...
        sbatch --job-name="w28_${short_task}_${seed}_${short_variant}" "$JOB_SCRIPT"
      )
